<x-crud>
    <form class="card-body text-center pb-2" action="{{ route('books.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('POST')
        ...
        <div class="mb-md-4 mt-md-4 pb-1">
            ...
            <div class="form-outline form-white mb-4 text-start align-items-center">
                ...
                <select type="text" id="author_id"
                    ...
                    name="author_id" required value="{{ old('author_id') }}">
            ...
            <div class="form-outline form-white mb-4 text-start">
                ...
                <input type="number" name="pages" class="form-control @error('pages') is-invalid @enderror"
                    id="password_confirmation" placeholder="0-9" value="{{ old('pages') }}" />
                @error('pages')
                    {{ $message }}
                @enderror
            </div>
            ...
            <div class="form-outline form-white mb-4 text-start">
                ...
                <input type="textarea" id="description"
                    ...
                    placeholder="Descrizione del libro" name="description" value="{{ old('description') }}" />
                @error('description')
                    {{ $message }}
                @enderror
            </div>
            ...
            <label for="password_confirmation" class="form-label">Prezzo <span class="text-danger">*</span></label>
                <input type="number" name="price" min="1"
                    ...
                    required placeholder="0-9€" value="{{ old('year') }}" />
                @error('price')
                    {{ $message }}
                @enderror
            </div>
